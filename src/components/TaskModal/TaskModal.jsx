...
import { DeleteTask, UpdateTaskApi } from '../../redux/actions/dashboard/dashboard.action';
...
const TaskModal = () => {
			...
			items={[
				{
					key: '2',
					label: (
						<p onClick={() => dispatch(DeleteTask(taskDetails?._id))} style={{ color: 'tomato' }}>
							Delete task
						</p>
					),
	...
	const handleUpdateTask = () => {
		dispatch(UpdateTaskApi(formData, formData?.comment, taskDetails?._id, taskDetails?._id));
	};
				...
				<Modal
					...
					title={
						<div style={{ display: 'flex', justifyContent: 'space-between' }}>
							<p style={{ color: '#333' }}>{taskDetails?.milestone_ref?.title}</p>
							<div
								style={{
									marginRight: 30,
									display: 'flex',
