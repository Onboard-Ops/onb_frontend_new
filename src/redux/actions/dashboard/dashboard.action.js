...
export const FetchCurrentMilestone = (projectID) => async (dispatch) => {
	try {
		...
		const res = await axios.get(`${API_URL}/get-all-milestone-by-current-project/${projectID}`, config);
		console.log(res);
		const {
			data: {
...
export const CreateTaskApi = (formData, comment) => async (dispatch) => {
	...
	if (comment?.commentBody) {
		...
		const response = await axios.post(`${API_URL}/create-task`, formData, config);
...
export const UpdateTaskApi = (formData, comment, taskID, taskRef) => async (dispatch) => {
	try {
		const currentProject = localStorage.getItem('currentProject');
		if (comment?.commentBody) {
			let commentid;
			...
			const res = await axios.put(`${API_URL}/update-task/${taskID}`, formData, config);
...
export const CreateMileStone = (formData) => async (dispatch) => {
	try {
		const res = await axios.post(`${API_URL}/create-milestone`, formData, config);
...
export const AddCustomerInfo = (formData, projectID) => async (dispatch) => {
	try {
		...
		const res = await axios.put(`${API_URL}/update-project/${projectID}`, payloadData, config);
		console.log(res);
